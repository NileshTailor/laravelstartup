		...
		<div class="portlet light ">
			<div class="portlet-title">
				<div class="caption">
					...
					<div class="actions">
									<a href="{{ URL::to('enquiryExcel/xls') }}"><button class="btn btn-success">Download Excel xls</button></a>
				<a href="{{ URL::to('enquiryExcel/xlsx') }}"><button class="btn btn-success">Download Excel xlsx</button></a>
				<a href="{{ URL::to('enquiryExcel/csv') }}"><button class="btn btn-success">Download CSV</button></a>					
	...
	<?php $i = 0; ?>
	  ...
	  <td><b>{{$i}}</b></td>
		 ...
		 <form action="{{action('USERController@enquirystatus')}}" method="post">
            ...
            <button class="btn btn-xs grey-cascade" style="{{$color}}" type="submit">{{$post['status']}}</button>
          </form></td>
		  <td>
		<a class="" data-toggle="modal" href="#basic{{$post['id']}}">
									<u>follow-up</u> </a>
									<div class="modal fade" id="basic{{$post['id']}}" tabindex="-1" role="basic" aria-hidden="true">
								<form method="post" action="{{action('USERController@followupcomment')}}">
								{{csrf_field()}}
